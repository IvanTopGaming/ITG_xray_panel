...
export default function AppButtons({ subUrl, lang }: { subUrl: string; lang: Lang }) {
  ...
  const MISSING_VISIBLE_MS = 12000;

  useEffect(() => {
    if (!missing) return;
    const timer = window.setTimeout(() => setMissing(null), MISSING_VISIBLE_MS);
  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {SUB_APPS.map((app) => (
          <button
            key={app.id}
            type="button"
            onClick={() => !isDesktop && open(app)}
            aria-disabled={isDesktop}
            aria-describedby={isDesktop ? 'apps-desktop-hint' : undefined}
            className={`flex items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.055] px-3 py-2.5 text-sm font-medium transition ${
              isDesktop ? 'cursor-not-allowed opacity-40' : 'hover:bg-white/[0.09]'
            }`}
          >
            {app.label}
          </button>
      ...
      {isDesktop && (
        <p id="apps-desktop-hint" className="mt-2.5 text-[11px] text-muted">
          {t('apps_desktop', lang)}
        </p>
      )}
      {missingApp && (
