...
**`panel-sub` is not only for the `sub` role.** `roles/master.py` still registers the `subscription` blueprint, so a master host needs `panel-sub` installed too. The cut is a *packaging* boundary, not a runtime one — the split-out distributions are strictly additive contributions to `panel_core`, and only `panel-botapi` is genuinely absent from the master's import graph.
...
2. **⚠️ …but there is no public route to the new webhook yet — this is the most important point.** `docker-compose.bot.yml` publishes **no ports** (no `ports:` key on either service), runs **no Caddy**, and puts both containers on a private `bot-net` bridge; `caddy/routes.yaml` has **no route to the bot host** (its three SNI routes are `proxy` → `xray:443`, `panel` → `frontend:80` + `backend:5000`, `sub` → `backend:5000`). So after this wave the webhook is **unreachable from the internet**, and payment confirmation degrades to the single 30-second `poll_pending_payments` cron. **Payments still complete, but confirmation takes up to 30s instead of being instant.** Publishing an ingress to bot-api is separate, not-yet-done work.
