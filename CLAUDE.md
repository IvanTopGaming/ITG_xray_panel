...
- `app/models.py` — SQLAlchemy models (20 total). Core: `Admin`, `Inbound`, `Client`, `Outbound`, `RoutingProfile`, `Balancer`, `SystemSetting`, `TrafficSnapshot`, `DomainStat`, `Node`, `NodeClientTraffic`, `ClientDevice`. Billing/bot: `Tariff`, `TariffItem`, `UserTariffAccess`, `Payment`, `BotText`, `BotEvent`, `TelegramUser`, `NotificationLog`
- `app/extensions.py` — Shared Flask extensions (db, migrate, APScheduler, Flask-Limiter)
- `app/utils.py` — JWT helpers + auth decorators: `token_required` (admin JWT only), `bot_service_token_required` (bot service token only), `admin_or_bot_token_required` (accepts either). The latter is used on `/api/inbound`, `/api/nodes`, and most `/api/system` endpoints — **but NOT on `/api/backup` and `/api/restore`** which take admin-only after the ultrareview hardening.
- `app/api/`
  - `auth` — login / logout
  - `inbound`, `outbound`, `routing`, `nodes`, `subscription`, `statistics`, `system` — core panel
...
- `app/services/`
  ...
  - `node_sync.py` — HTTP client to remote nodes, user/inbound/traffic sync, group filtering, aggregated subscription links
  ...
  - `provisioning.py` — single gateway for tariff grants: extends an existing `Client` for the same (telegram_id, inbound_tag) or creates one; resets counters; clears `traffic_*` `NotificationLog`; syncs to nodes
...
- `pages/` — `Dashboard` (inbound/outbound management), `Statistics` (traffic analytics), `Routing`, `Nodes`, `Bot` (billing UI), `System` (settings + logs + backup + about), `Login`
...
- `api_service.py` — multi-panel manager (`MultiPanelManager`); discovers nodes via master's `/api/nodes` and "shadows" any standalone entry pointing at the same node, so user write ops aren't double-sent
...
### Traffic enforcement
`stats.py` polls per-user up/down via Xray gRPC every 10s, writes to `Client.up`/`down` and upserts hourly `TrafficSnapshot` rows. `check_limits` (60s) removes users that exceed limit or expiry using traffic aggregated across the master and every node's `NodeClientTraffic`. Monthly resets (per-client `reset_day`) zero the counters **and** delete that client's `traffic_*` `NotificationLog` rows so the next cycle's warnings can fire.
...
| `check_limits` | 60s | Removes expired/over-limit users (sums local + node traffic) |
| `parse_logs` | 15s | Tails Xray access logs into `DomainStat` (skips bare IPs) |
| `cleanup_stats` | 24h | Deletes `DomainStat` rows > 90d |
| `node_health_check` | 60s | Pings each enabled node |
| `node_traffic_poll` | 60s | Pulls per-user counters from nodes into `NodeClientTraffic` |
| `node_inbound_sync` | 5m | Pushes inbound config to nodes with `sync_inbound=True` |
| `node_user_sync` | 1h | Full reconcile of users on each node (adds/updates/deletes if `strict_mirror`) |
...
### Auth
Three decorators in `app/utils.py`:
...
- `admin_or_bot_token_required` — accepts either. Used on `/api/inbound`, `/api/nodes`, parts of `/api/system` (e.g. `/api/restart`, `/api/stats/system`) — needed because the bot legitimately needs to create/update/delete users.
...
Single `_sync_after_provision` call after the loop: regenerates Xray config, restarts container if needed, fans the change out to nodes via `node_sync`, invalidates the Redis sub-cache.

### Multi-node architecture

The panel can act as a *master* that manages remote panel *nodes*. `Node` rows store URL/credentials/inbound_tag plus per-node flags: `sync_users` (push user CRUD), `sync_inbound` (push inbound configs), `strict_mirror` (delete remote users not present on master), and a comma-separated `groups` list. `Client.allowed_node_groups` (also CSV) restricts which nodes a user is provisioned on — empty means "all nodes".

Live user CRUD in `api/inbound.py` and `api/bot_admin.py` calls `sync_user_create/update/delete` from `node_sync.py` to fan out immediately; the hourly `node_user_sync_job` is the safety net that reconciles drift. `NodeClientTraffic` stores the latest absolute up/down per (node, email); enforcement and statistics sum these with the master's local `Client.up/down`. Subscription links (`api/subscription.py`) merge per-protocol entries from every node visible to the requesting client.

**Migration note:** schema v13 is *not* backwards-compatible with v9 — a master on v13 pushes new user/tariff structures that a v9 node can't parse. **Upgrade master and nodes in the same deploy.**
...
- Admin can `block` a user (`POST /bot/users/<id>/block`): cancels all `UserTariffAccess` grants, disables all `Client` rows, **removes them from Xray runtime via gRPC for vless/vmess (otherwise triggers config regen + restart)**, and propagates the deletion to every node via `sync_user_delete`. `unblock` only clears the flag — does **not** restore cancelled tariffs or re-enable clients.
...
### Subscription links
`api/subscription.py` serves `GET /api/sub/<uuid_str>` — UUID-keyed, so renaming `Client.email` does NOT break a user's existing app config. The response merges aggregated entries from every node visible to the user. Cached in Redis with a configurable TTL (`subscription_update_interval_hours` SystemSetting).
...
### Multi-node deploy ordering
When the schema bumps (any `CURRENT_DB_VERSION` change), **deploy master and all nodes in the same wave**. A master on a newer schema may push user/tariff structures that an older node can't parse. Backup first (`GET /api/backup`), then `docker compose pull && up -d` everywhere.
