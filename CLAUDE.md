...
In the split Postgres deployment, `PANEL_ROLE` selects one of four Flask app factories (`panel_core.roles.{master,worker,sub,botapi}`): `master` (default — billing/admin API, no local Xray), `worker` — called a **node** below — which runs its own Xray plus its own inbound/client rows and local Redis, `sub` (serves subscription links only), and `bot` (serves `/bot-service/*` for the Telegram bot, i.e. **bot-api**). A **node** here is *not* a Panel Federation `LinkedPanel` (see Panel Federation below): a `LinkedPanel` is a separate whole panel talking over HTTP with its own SQLite, while a node is one role sharing the same Postgres as the rest of a single split deployment.
