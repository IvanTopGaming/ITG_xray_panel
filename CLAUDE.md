...
**`panel-sub` is not only for the `sub` role — and `panel-core` cannot build its own roles without it.** `roles/master.py` and `roles/worker.py` ship from `panel-core` but both register the `subscription` blueprint, which ships from `panel-sub`. `panel-sub` declares `dependencies = ["panel-core"]`, so the reverse edge cannot be declared without a workspace cycle: this is a genuine **dependency inversion**, not merely "the master also needs panel-sub installed". Install `panel-core` alone and `panel_core.roles.master.create_app()` dies with `ImportError: cannot import name 'subscription' from 'panel_core.api' (unknown location)`. Production is unaffected (`backend/Dockerfile` installs the whole workspace, so all three distributions are present in every image), and the inversion disappears for free at the `panel-master`/`panel-worker` cut, when `roles/{master,worker}.py` move out of `panel-core` into distributions that *may* depend on `panel-sub`. **The checkable exit criterion of that cut is "`ALLOWED_INVERSIONS` is empty"** — not the literal "`panel-core` imports nothing from another distribution", which is unreachable: `dispatch.py` also ships from `panel-core` and must import `roles/{sub,botapi}` to dispatch to them, which is exactly why the guard carries a **separate** set, `ROLE_DISPATCH_EXEMPTIONS`, for that edge. The two sets differ in kind, not just in name: `ALLOWED_INVERSIONS` is transitional and designed to collapse to zero at the `panel-master`/`panel-worker` cut (see below); `ROLE_DISPATCH_EXEMPTIONS` is structural and permanent — do not attempt to empty it too. Until the cut lands, the two `ALLOWED_INVERSIONS` edges are a named allowlist in `tests/test_distribution_imports.py`, and a staleness test there fails once they stop happening, so whoever does the cut is told to delete them. Only `panel-botapi` is genuinely absent from the master's import graph.
