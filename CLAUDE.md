...
**Where the code actually lives.** The backend is a uv workspace (`backend/pyproject.toml` → `[tool.uv.workspace] members = ["packages/*"]`) whose only member today is `packages/panel-core`. Every `app/…` path in the list below is shorthand for `backend/packages/panel-core/src/panel_core/…` — e.g. `app/models.py` is `packages/panel-core/src/panel_core/models.py`, imported as `panel_core.models`.

**`panel_core` is a namespace package (PEP 420).** Neither it nor its splittable subpackages (`api/`, `services/`, `jobs/`, `roles/`, `xray/`, `data/`) carries an `__init__.py`, so a future package cut can ship them from separate distributions into the same import root without a single one of the ~880 existing `panel_core.*` import statements changing. Consequences you must not undo (guarded by `tests/test_namespace_packages.py`, `tests/test_xray_facade.py`, `tests/test_bootstrap.py`):
- **Importing `panel_core` runs no code.** What the deleted `__init__.py` files held now lives in explicit modules: `bootstrap.py` (`bootstrap_gevent()` — `gevent.monkey.patch_all()` + `patch_gevent_psycopg()`), `dispatch.py` (`create_app()`, the `PANEL_ROLE` → role-module dispatcher) and `xray/facade.py` (the gateway shims `has_local_xray`, `generate_config_file`, `restart_xray_container`, `stream_xray_logs`, `update_geo_db`, `_api_add_user_grpc`, `_api_remove_user_grpc`). Import them from those modules — `from panel_core.xray import generate_config_file` and `from panel_core import xray` + `xray.generate_config_file` both break once the namespace spans two distributions, and both forms are rejected by the guard.
