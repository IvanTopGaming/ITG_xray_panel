from panel_core.services.xray import _extract_reason
...
import pytest
from panel_core.services import xray as xray_svc


def test_validate_skips_when_binary_absent():
    with patch("panel_core.services.xray.os.path.exists", return_value=False):
        with patch("panel_core.services.xray.subprocess.run") as run:
            xray_svc._validate_xray_config("/tmp/c.json")
    run.assert_not_called()


def test_validate_ok_on_exit_zero():
    with patch("panel_core.services.xray.os.path.exists", return_value=True):
        with patch(
            "panel_core.services.xray.subprocess.run",
...
class TestGenerateConfigGate:
    ...
    def test_valid_config_is_committed(self):
        ...
        with patch("panel_core.services.xray._validate_xray_config"):
    ...
    def test_rejected_config_is_not_committed_and_candidate_removed(self):
        ...
        with patch(
            "panel_core.services.xray._validate_xray_config",
