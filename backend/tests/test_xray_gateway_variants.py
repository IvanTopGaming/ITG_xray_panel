...
local = importlib.import_module("panel_core.xray.local")
...
report["control"] = exercise(local.LocalXrayGateway(), [("apply_config", ())], False)
...
def test_gateway_protocol_covers_counter_resets():
    from panel_core.xray.gateway import NullXrayGateway, RemoteXrayGateway, XrayGateway
