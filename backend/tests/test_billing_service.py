"""Unit tests for billing.create_checkout and billing.apply_payment.

YooKassa SDK is mocked at the module level — no network calls.
"""
...
def test_apply_payment_marks_failed_when_tariff_archived_between_checkout_and_webhook(app, public_tariff):
    """Race: user starts checkout while tariff is public, admin archives it,
    then YooKassa webhook arrives. We must NOT provision an archived tariff —
...
def test_apply_payment_skips_when_row_no_longer_pending(app, public_tariff):
    """Webhook + poll cron can both reach apply_payment with the same row.
    The fix uses an atomic UPDATE … WHERE status='pending' to claim the row;
    the loser of the race sees rowcount=0 and must not call provisioning.

    Simulates the lost race by pre-flipping DB status to 'processing' (as if
    a concurrent caller had just claimed it). The ORM instance also reflects
    'processing' after refresh — the in-memory fast-path check `== "succeeded"`
    does NOT short-circuit, so without the row-level claim the code would
    fall through and double-provision."""
...
def test_apply_payment_marks_failed_when_private_tariff_lost_grant(app, public_tariff):
    """Symmetric to archived-race: tariff flipped to 'private' and user lost
    their UserTariffAccess between checkout and webhook. _ensure_tariff_available
